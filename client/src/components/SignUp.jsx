...
const SignUp = () => {
  ...
  const handleSubmit = (event) => {
    ...
    axios.post(`${domain}signup`, { username, password })
      .then((response) => {
    ...
    <Grid container>
      ...
      <Grid item xs={6}>
        <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', height: '100vh', justifyContent: 'center' }}>
          ...
          />
          <Button type="submit" onClick={navigate('/login')} variant="contained" fullWidth size="large" sx={{ mt: 2, bgcolor: 'black', color: 'white', '&:hover': {backgroundColor: 'black'}, }}>
            Sign Up
          </Button>
        </Box>
      </Grid>
